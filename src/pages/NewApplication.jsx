...
export default function NewApplication() {
  ...
  const [currentDocForUpload, setCurrentDocForUpload] = useState(null);
  const navigate = useNavigate();
  const { toast } = useToast();

  const update = (field, value) => setForm((f) => ({ ...f, [field]: value }));

  const handleFileSelect = (file, docName) => {
    const maxSize = 20 * 1024 * 1024; // 20MB
    const allowedTypes = ["application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "image/png", "image/jpeg"];
  ...
  const handleUploadClick = (docName) => {
    setCurrentDocForUpload(docName);
    fileInputRef.current?.click();
  };

  const handleFileInputChange = (e) => {
    const file = e.target.files?.[0];
    if (file && currentDocForUpload) {
      handleFileSelect(file, currentDocForUpload);
    }
    // Reset the input
        ...
        >
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              ...
              {["Environmental Impact Assessment", "Site Layout Plan", "Consent Documents", "Project Feasibility Report"].map((doc) => (
                <div
                  ...
                  className={`p-4 rounded-lg border-2 transition-colors cursor-pointer ${
                    dragOverDoc === doc ? "border-primary bg-primary/5" : "border-border bg-muted/30"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      {uploadedFiles[doc] ? (
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <div>
                            <p className="text-xs text-green-600 font-medium">{uploadedFiles[doc].name}</p>
                            <p className="text-xs text-muted-foreground">{uploadedFiles[doc].size} MB</p>
                          </div>
                        </div>
                      ) : (
                        <p className="text-xs text-muted-foreground mt-1">PDF, DOCX, XLSX, PNG, JPG — Max 20MB</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        variant={uploadedFiles[doc] ? "outline" : "default"}
                        size="sm"
                        onClick={() => handleUploadClick(doc)}
                        className="gap-2"
                      >
                        <Upload className="h-4 w-4" />
                        {uploadedFiles[doc] ? "Replace" : "Upload"}
                      </Button>
                    </div>
                  </div>
                </div>
              ...
              <p className="text-xs text-muted-foreground p-3 bg-blue-50 rounded-lg border border-blue-200 text-blue-700">
